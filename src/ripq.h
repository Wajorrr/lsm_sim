...
protected:
  ...
  public:
    item(const Request req, block_ptr blk)
        : req(req), physical_block(blk), virtual_block(blk),
          is_ghost(false), in_dram(false)
    {
    }
    ...
    item(const item &obj) : std::enable_shared_from_this<item>(), req(obj.req), physical_block(obj.physical_block), virtual_block(obj.virtual_block), is_ghost(obj.is_ghost), in_dram(obj.in_dram)
    {
    }
  ...
  // std::enable_shared_from_this 是一个模板类，它在 C++ 标准库中
  // 用于提供一个对象能够从其自身生成 std::shared_ptr 的机制
  class block : public std::enable_shared_from_this<block>
  {
  public:
    block(section_ptr s, bool is_virtual)
        : s(s), items{}, filled_bytes(0), is_virtual(is_virtual),
          active(true), num_items(0), id(global_bid)
    {
      global_bid++;
    }
    ...
    block(const block &obj) : std::enable_shared_from_this<block>(), s(obj.s),
                              items(obj.items), filled_bytes(obj.filled_bytes),
                              is_virtual(obj.is_virtual), active(obj.active),
                              num_items(obj.num_items), id(obj.id)
    {
    }
  ...
  public:
    ...
    section(int id, stats &stat)
        : blocks{}, active_phy_block{}, active_vir_block{},
          id(id), filled_bytes(0), stat(stat)
    {
    }
    ...
    section(const section &obj) : std::enable_shared_from_this<section>(), blocks(obj.blocks), active_phy_block(obj.active_phy_block), active_vir_block(obj.active_vir_block), id(obj.id), filled_bytes(obj.filled_bytes), stat(obj.stat)
    {
    }
